...
class PSEO_Ajax {
    ...
    public function save_project(): void {
        ...
        $data = [
            ...
            'source_type'   => sanitize_key( $_POST['source_type'] ?? 'csv_url' ),
            'source_config' => wp_json_encode( json_decode( stripslashes( $_POST['source_config'] ?? '{}' ), true ) ),
            'url_pattern'   => sanitize_text_field( $_POST['url_pattern'] ?? '' ),
            'seo_title'     => sanitize_text_field( $_POST['seo_title'] ?? '' ),
            'seo_desc'      => sanitize_textarea_field( $_POST['seo_desc'] ?? '' ),
            'robots'        => sanitize_text_field( $_POST['robots'] ?? 'index,follow' ),
            'schema_type'   => sanitize_text_field( $_POST['schema_type'] ?? '' ),
            'sync_interval' => sanitize_key( $_POST['sync_interval'] ?? 'manual' ),
        ];
